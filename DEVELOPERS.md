...
```shell
git clone git@github.com:ion-gateway/sdk.git && cd sdk
```
...
Next, link the `@ion-gateway/ui-react` package to your project:
...
Now, build the `@ion-gateway/ui-react` package in watch mode:
...
Update the version of the packages in the following order, one at a time:
 ...
 1. `@ion-gateway/isomorphic-eventsource`
 2. `@ion-gateway/isomorphic-fetch`
 3. `@ion-gateway/protocol`
 4. `@ion-gateway/sdk`
 5. `@ion-gateway/ui`
 6. `@ion-gateway/ui-react`
...
For example, if changes were made in `@ion-gateway/ui` and `@ion-gateway/ui-react`, you should first run the following for `@ion-gateway/ui`:
...
Next, publish the version of the package. For `@ion-gateway/ui`:
...
After publishing the version of `@ion-gateway/ui`, push the commit and tags:
...
After publishing the version of `@ion-gateway/ui`, update its version in the `dependencies` section of `@ion-gateway/ui-react`'s `package.json` file.

 ```json
 /* file: packages/ui-react/package.json */
 {
   "dependencies": {
     "@ion-gateway/ui": "CURRENT_VERSION"
   }
 }
 ```

> TODO: add a step to run `npm install` in `@ion-gateway/ui-react` for updating `package-lock.json` file when the step will be tested.

Then, create a "chore" commit to save this change:

 ```shell
 git add packages/ui-react/package.json
 git commit -m "chore(ui-react): update @ion-gateway/ui to CURRENT_VERSION"
 ```

#### 6. Repeat Steps for Next Package (if needed)

After publishing the version, repeat the [above steps](#publish-a-version) for the next package in the list, e.g. `@ion-gateway/ui-react`.
...
If you don't have the [demo-dapp-with-wallet](https://github.com/ion-gateway/demo-dapp-with-wallet) repository, clone it:

```shell
git clone git@github.com:ion-gateway/demo-dapp-with-wallet.git && cd demo-dapp-with-wallet
```
...
Update the `@ion-gateway/ui-react` package version in the `package.json` file:

```json
{
  "dependencies": {
    "@ion-gateway/ui-react": "CURRENT_VERSION"
  }
}
```
...
```shell
git add . && git commit -m "chore: update @ion-gateway/ui-react to CURRENT_VERSION" && git push origin HEAD
```

> Caution: Be careful when updating any demo apps, as they are used for testing by the community.

After updating the demo app, test the beta version of the package in the [demo app](https://ion-gateway.github.io/demo-dapp-with-wallet/). If everything works correctly, you are ready to publish a new release. **Don't forget to repeat the steps for the release version.**

> Note: Currently, the beta version is tested on the [demo-dapp-with-wallet](https://github.com/ion-gateway/demo-dapp-with-wallet). However, it is recommended to also update the [demo-dapp-with-react-ui](https://github.com/ion-gateway/demo-dapp-with-react-ui) and use it for testing.

# Additional Information

## CDN Access

You can access the package via CDN at the following URL: [https://unpkg.com/@ion-gateway/sdk@latest/dist/ionconnect-sdk.min.js](https://unpkg.com/@ion-gateway/sdk@latest/dist/ionconnect-sdk.min.js).

## Demo DApps

The package can be tested using the following demo DApps:

- [demo-dapp-with-wallet](https://github.com/ion-gateway/demo-dapp-with-wallet): Currently utilized for testing beta versions. It needs to be replaced with demo-dapp-with-react-ui.
- [demo-dapp-with-react-ui](https://github.com/ion-gateway/demo-dapp-with-react-ui): Currently outdated. It requires an update following the release publication.
- [demo-dapp](https://github.com/ion-gateway/demo-dapp): Currently outdated. It requires an update.
